...
// Stress the exact failure mode: optional/Firebase network can be completely
// unavailable. A post-login overlay must still never trap the Android dashboard.
...
async function snapshot(label) {
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      splashExists: Boolean(splash),
      splashBlocking: Boolean(splash && splashStyle &&
        splashStyle.display !== 'none' &&
        splashStyle.visibility !== 'hidden' &&
        Number(splashStyle.opacity) > 0 &&
        splashStyle.pointerEvents !== 'none'),
      shieldExists: Boolean(shield),
      shieldBlocking: Boolean(shield && shieldStyle &&
        shieldStyle.display !== 'none' &&
        shieldStyle.visibility !== 'hidden' &&
        Number(shieldStyle.opacity) > 0 &&
        shieldStyle.pointerEvents !== 'none'),
      ...
      mineLabel: String(document.getElementById('btnText')?.textContent || '').trim(),
  ...
  assert.equal(state.shieldExists, false, `${label}: Android should never create secondary startup shield`);
  assert.equal(state.homeActive, true, `${label}: mining/home screen is not active`);
  assert.equal(state.mineVisible, true, `${label}: mining control not visible`);
  assert.equal(state.dockVisible, true, `${label}: bottom dock not visible`);
  assert.notEqual(state.balance, '0.0000', `${label}: stale fake zero balance exposed`);
}

// No helper in this test hides/removes the splash. Every check observes the page
// as actually prepared by the Android build pipeline.
...
// Prove the dashboard is touchable while all external network is blocked.
...
await context.close();
await browser.close();
console.log('PASS Android dashboard startup deep test: no post-login blocking splash/shield, no stale 0.0000, dashboard remains touchable with external network fully blocked.');
