...
def gate(w,h):
    ...
    result=ev(ws,n,r'''(()=>{const d=document.documentElement,b=document.body,t=document.querySelector('.nxf-travel');const R=e=>{if(!e)return null;const r=e.getBoundingClientRect();return{l:r.left,r:r.right,t:r.top,b:r.bottom,w:r.width,h:r.height}};const V=(s,m)=>{const r=R(document.querySelector(s));return!!r&&r.w>0&&r.h>=m&&r.l>=-.5&&r.r<=innerWidth+.5&&r.t>=-.5&&r.b<=innerHeight+.5};const C=s=>{const r=R(document.querySelector(s));return!!r&&r.w>0&&r.r<=innerWidth+.5&&r.l>=-.5};const text=t.innerText;const req=['NEXUSNOVA TRAVEL','Flights','Buses','Trains','Hotels','Trip','Compare before you book.','SEARCH FLIGHTS','Fare Calendar','Disruption Rescue','Flight status'];const bg=[getComputedStyle(t).backgroundColor,getComputedStyle(t.querySelector('.nxf-head')).backgroundColor,getComputedStyle(t.querySelector('.nxf-primary')).backgroundColor];const rgb=s=>{const m=String(s).match(/(\d+)[^\d]+(\d+)[^\d]+(\d+)/);return m?[+m[1],+m[2],+m[3]]:null};const c=bg.map(rgb).filter(Boolean);return{width:innerWidth,height:innerHeight,docW:d.scrollWidth,bodyW:b.scrollWidth,labels:req.every(x=>text.includes(x)),dark:c.some(([r,g,b])=>Math.max(r,g,b)<80),blue:c.some(([r,g,b])=>b>r*1.12&&b>g*1.08),emerald:c.some(([r,g,b])=>r<30&&g>70&&g>r*2&&g>b*1.25),back:V('.nxf-back',36),pk:V('[data-pk]',36),external:V('[data-browser]',36),origin:V('[data-pick="origin"]',50),destination:V('[data-pick="destination"]',50),swap:V('[data-swap]',30),search:V('[data-search]',40),calendar:V('[data-calendar]',30),rescue:V('[data-rescue]',30),status:C('[data-flight]'),footer:V('.nxf-footer',1)}})()''')
