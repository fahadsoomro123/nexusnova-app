/* NexusNova NOVA AI POWER V6 mobile controls
...
(() => {
  ...
  function select(mode,preset=''){
    if(['website','dev','builder','power'].includes(mode)&&!paired()){
      status(`${mode==='builder'?'App Builder':mode==='power'?'Power V6':mode} ke liye Local AI pair karo`);openSettings();return false;
    }
    save({mode,reasoning:['research','power','builder'].includes(mode)?'deep':'auto',novaVersion:'6'});sync();if(preset)setInput(preset);return true;
  }
  ...
  function ensureModes(){
    ...
    ensureMode(modes,'power','Power V6','nx-v6-mode');
  }
  ...
  function ensureBadge(){
    ...
    const b=document.createElement('span');b.className='nx-v6-badge';b.innerHTML='<b>✦</b> POWER V6';title.appendChild(b);
  }
  ...
  function enhancePlus(){
    ...
    if(!menu.querySelector('[data-v6="builder"]')){
      const b=document.createElement('button');b.className='nx-nova-plus-item';b.dataset.v6='builder';b.innerHTML='<span style="width:18px;text-align:center">▣</span><span>Build a New App<small class="nx-v6-mini">Plan → create → code → build/test → review → export</small></span>';
      b.onclick=e=>{e.preventDefault();e.stopPropagation();menu.remove();select('builder','APP BUILDER V6: Is app ko complete working project ki surat me banao. Requirements khud clarify karo sirf jab technically unavoidable ho; warna sensible defaults use karo. Project inspect/scaffold karo, core features implement karo, build/check run karo, reviewer/critic issues fix karo aur downloadable project export ke liye ready karo. App idea: ');};menu.appendChild(b);
    }
    if(!menu.querySelector('[data-v6="power"]')){
      const b=document.createElement('button');b.className='nx-nova-plus-item nx-v6-mode';b.dataset.v6='power';b.innerHTML='<span style="width:18px;text-align:center">✦</span><span>Power V6 Task<small class="nx-v6-mini">Context → Planner → Worker → Reviewer → Critic → recovery</small></span>';
      b.onclick=e=>{e.preventDefault();e.stopPropagation();menu.remove();select('power','POWER V6: Is complex task ko end-to-end khud solve karo. Project context aur zaroorat par fresh web research use karo; execute, build/test, diff/evidence verify karo; reviewer/critic fail kare to correction passes chalao. Task: ');};menu.appendChild(b);
    }
  }
  ...
  function sync(){
    ...
    if(sub){
      if(c.mode==='builder')sub.textContent=paired()?'App Builder V6 • create → build → review':'App Builder V6 • local AI required';
      else if(c.mode==='power')sub.textContent=paired()?'Power V6 • planner → worker → reviewer → critic':'Power V6 • local AI required';
    ...
    if(st&&!/working|thinking|local .*•/i.test(st.textContent||'')){
      if(c.mode==='builder')st.textContent=paired()?'App Builder V6 ready':'Pair Local AI for App Builder';
      else if(c.mode==='power')st.textContent=paired()?'Power V6 ready':'Pair Local AI for Power V6';
    }
  }
  ...
  window.NexusNovaV6={select,sync,paired,version:'6.0.0'};
})();
