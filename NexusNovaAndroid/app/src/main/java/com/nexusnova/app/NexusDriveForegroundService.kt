 ...
 * Route points are persisted incrementally so a process restart keeps the active
 * trip geometry, while completed records remain backward-compatible aggregates.
 */
class NexusDriveForegroundService : Service(), LocationListener {
    ...
    private fun restoreSnapshot() {
        ...
        speedKmh = 0.0
        lastFix = null
        restoreRoutePoints(snapshot.optJSONArray("routePoints"))
        stationarySince = 0L
    ...
    override fun onLocationChanged(location: Location) {
        ...
        evaluateActiveTrip(time, acceptedDistance, dt, liveKmh, footActivity, current)
    ...
    private fun startTrip(mode: String, now: Long, liveKmh: Double, seedFix: Fix?) {
        ...
        stationarySince = 0L
        routePoints.clear()
        seedFix?.let { appendRoutePoint(it, force = true) }
        lastStatus = if (mode == MODE_BICYCLE) "Bicycle trip started automatically" else "Vehicle trip started automatically"
        resetCandidate()
    }

    private fun evaluateActiveTrip(now: Long, acceptedDistance: Double, dt: Long, liveKmh: Double, footActivity: Boolean, current: Fix) {
        if (footActivity) {
            speedKmh = 0.0
            markStationary(now)
        ...
        if (meaningfulMove) {
            ...
            movingMs += max(0L, dt)
            appendRoutePoint(current, force = routePoints.isEmpty())
    ...
    private fun finishCurrentTrip(now: Long, reason: String) {
        ...
        if (meaningful) {
            ...
            val completed = JSONObject()
                ...
                .put("mode", tripMode)
                .put("routePoints", routeJson())
                .put("startPoint", routePointJson(routePoints.firstOrNull()))
                .put("endPoint", routePointJson(routePoints.lastOrNull()))
    ...
    private fun appendRoutePoint(fix: Fix, force: Boolean = false) {
        if (!active) return
        if (!fix.lat.isFinite() || !fix.lon.isFinite()) return
        if (kotlin.math.abs(fix.lat) > 90.0 || kotlin.math.abs(fix.lon) > 180.0) return
        if (!fix.accuracy.isFinite() || fix.accuracy > ROUTE_MAX_ACCURACY_M) return

        val speed = if (fix.speedMps.isFinite()) (fix.speedMps * 3.6).coerceIn(0.0, MAX_DRIVE_KMH) else finiteOrZero(speedKmh)
        val last = routePoints.lastOrNull()
        if (!force && last != null) {
            val gap = fix.at - last.at
            if (gap < ROUTE_MIN_GAP_MS) return
            val results = FloatArray(1)
            Location.distanceBetween(last.lat, last.lon, fix.lat, fix.lon, results)
            val moved = results[0].toDouble()
            if (moved < ROUTE_MIN_DISTANCE_M && gap < ROUTE_FORCE_GAP_MS) return
        }

        routePoints.add(RoutePoint(fix.lat, fix.lon, fix.at, fix.accuracy, speed))
        trimRoutePoints()
    }

    private fun trimRoutePoints() {
        if (routePoints.size <= MAX_ROUTE_POINTS) return
        val keep = mutableListOf<RoutePoint>()
        routePoints.forEachIndexed { index, point ->
            if (index == 0 || index == routePoints.lastIndex || index % 2 == 0) keep.add(point)
        }
        routePoints.clear()
        routePoints.addAll(keep.takeLast(MAX_ROUTE_POINTS))
    }

    private fun routePointJson(point: RoutePoint?): Any =
        point?.let {
            JSONObject()
                .put("lat", it.lat)
                .put("lng", it.lon)
                .put("at", it.at)
                .put("accuracy", finiteOrZero(it.accuracy))
                .put("speedKmh", finiteOrZero(it.speedKmh))
        } ?: JSONObject.NULL

    private fun routeJson(): JSONArray {
        val route = JSONArray()
        routePoints.forEach { route.put(routePointJson(it)) }
        return route
    }

    private fun restoreRoutePoints(raw: JSONArray?) {
        routePoints.clear()
        if (raw == null) return
        for (index in 0 until min(raw.length(), MAX_ROUTE_POINTS)) {
            val item = raw.optJSONObject(index) ?: continue
            val lat = item.optDouble("lat", Double.NaN)
            val lon = item.optDouble("lng", item.optDouble("lon", Double.NaN))
            val at = item.optLong("at", 0L)
            val accuracy = item.optDouble("accuracy", Double.NaN)
            val speed = item.optDouble("speedKmh", 0.0).safeNonNegative()
            if (!lat.isFinite() || !lon.isFinite() || kotlin.math.abs(lat) > 90.0 || kotlin.math.abs(lon) > 180.0 || at <= 0L) continue
            routePoints.add(RoutePoint(lat, lon, at, if (accuracy.isFinite()) accuracy else 0.0, speed))
        }
    }
    ...
    private fun enqueueCompletedTrip(completed: JSONObject) {
        ...
        queue.put(completed)
        // Do not evict older pending trips by count. Pending records are retained
        // until the web/account persistence layer has had a chance to import them.
        prefs().edit().putString(KEY_COMPLETED_QUEUE, queue.toString()).apply()
    }
    ...
    companion object {
        ...
        // Keep the breadcrumb resilient on vendor GPS stacks (including XOS)
        // while retaining the same 45 m quality ceiling used by trip tracking.
        private const val ROUTE_MAX_ACCURACY_M = 45.0
        private const val ROUTE_MIN_GAP_MS = 2_000L
        private const val ROUTE_FORCE_GAP_MS = 10_000L
        private const val ROUTE_MIN_DISTANCE_M = 5.0
