 ...
 *
 * The bundled web app runs on appassets.androidplatform.net. Web reCAPTCHA is not
 * used there; instead the web Firebase SDK asks this native bridge for an Android
 * App Check token and consumes it through a CustomProvider.
 */
object NexusNativeAppCheck {
    ...
    fun handleMessage(payload: String?, replyProxy: JavaScriptReplyProxy) {
        val request = try {
            ...
            replyError(replyProxy, "", "Invalid App Check request")
            return
        }

        if (request.optString("action") != ACTION_GET_TOKEN) return
