...
async function makePdf(file){
  ...
  const blob=new Blob([await pdf.save()],{type:'application/pdf'}),url=URL.createObjectURL(blob),a=document.createElement('a'),base=String(file.name||'document').replace(/\.[^.]+$/,'').replace(/[^a-z0-9._-]+/gi,'-')||'document';
...
export function renderDocumentsLiveSuite(){
  ...
  root.prepend(panel);
  const input=panel.querySelector('[data-live-file]'),status=panel.querySelector('[data-live-status]'),result=panel.querySelector('[data-live-result]');
  input.addEventListener('change',()=>{const f=input.files?.[0];status.textContent=f?`${f.name} • ${(f.size/1024/1024).toFixed(2)} MB`:'No image selected.';});
  panel.querySelector('[data-receipt]').addEventListener('click',async()=>{
    const f=input.files?.[0];if(!f||!/^image\/(jpeg|png|webp)$/i.test(f.type)){status.textContent='Choose a JPG, PNG or WebP receipt first.';return;}status.textContent='Analyzing receipt with Nova AI…';result.hidden=true;
    try{const [m,d]=await Promise.all([model(),inline(f)]),r=await m.generateContent([{inlineData:d},{text:'Analyze this receipt image. Extract merchant/store, date, currency, subtotal, tax, total, payment method if visible, and item lines if readable. Do not invent unreadable values; mark them unclear.'}]),text=String(r?.response?.text?.()||'').trim();if(!text)throw new Error('AI returned no receipt analysis.');result.querySelector('p').textContent=text;result.hidden=false;status.textContent='Receipt analysis complete.';}catch(e){status.textContent=/app.?check|403|permission/i.test(String(e?.message||''))?'Receipt AI was blocked by Firebase App Check / provider configuration.':String(e?.message||'Receipt analysis unavailable.').slice(0,260);}
  });
  panel.querySelector('[data-pdf]').addEventListener('click',async()=>{const f=input.files?.[0];if(!f||!/^image\//i.test(f.type)){status.textContent='Choose an image first.';return;}if(f.size>20*1024*1024){status.textContent='Choose an image smaller than 20 MB.';return;}status.textContent='Creating real PDF locally…';try{await makePdf(f);status.textContent=`PDF created locally from ${f.name}.`;}catch(e){status.textContent=String(e?.message||'PDF creation failed.').slice(0,260);}});
  return root;
}
