...
export function renderNovaDrivePremium() {
  ...
  const onFix=position=>{
    ...
    if(!Number.isFinite(accuracy)||accuracy>MAX_GPS_ACCURACY_M){
      stateEl.textContent='GPS ACQUIRING'; status.textContent='Waiting for a precise GPS fix…'; lastFix=null; ride.speedKmh=0; paint(); return;
    }
  ...
  const finish=()=>{
    ...
    stateEl.textContent='SAVED'; status.textContent='Drive stopped • aggregate trip summary saved • coordinates were not stored.';
...
function aggregateRange(store,startAt){
  ...
  return Object.entries(store.days||{}).reduce((sum,[key,row])=>{
    ...
    sum.distanceM+=Number(row.distanceM)||0; sum.movingMs+=Number(row.movingMs)||0; sum.trips+=Number(row.trips)||0; return sum;
...
export function renderNovaTrackPremium(){
  ...
  const draw=()=>{
    if(!key) return;
    const store=readDriveStore(key),today=store.days?.[localDayKey()]||{distanceM:0,movingMs:0,trips:0},week=aggregateRange(store,weekStart()),month=aggregateRange(store,monthStart());
    todayEl.textContent=formatDistance(today.distanceM||0); weekEl.textContent=formatDistance(week.distanceM); monthEl.textContent=formatDistance(month.distanceM); timeEl.textContent=formatDuration(today.movingMs||0);
    const days=[];
    for(let i=6;i>=0;i-=1){const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()-i);const row=store.days?.[localDayKey(d)]||{distanceM:0,trips:0};days.push({label:d.toLocaleDateString(undefined,{weekday:'short'}).slice(0,2).toUpperCase(),distanceM:Number(row.distanceM)||0,trips:Number(row.trips)||0});}
    const max=Math.max(1,...days.map(day=>day.distanceM)),total=days.reduce((sum,day)=>sum+day.distanceM,0); totalEl.textContent=formatDistance(total);
    barsEl.innerHTML=days.map(day=>{const height=Math.max(5,day.distanceM/max*100);return `<article title="${escapeHtml(formatDistance(day.distanceM))} • ${day.trips} trips"><div><i style="height:${height.toFixed(2)}%"></i></div><strong>${escapeHtml(day.label)}</strong><span>${day.distanceM>0?(day.distanceM/1000).toFixed(1):'0'}</span></article>`;}).join('');
    tripsEl.innerHTML=store.trips.length?store.trips.slice(0,12).map(trip=>`<article class="nxtrack-trip"><div><strong>${escapeHtml(new Date(trip.at).toLocaleDateString([],{day:'2-digit',month:'short'}))}</strong><span>${escapeHtml(new Date(trip.at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}))}</span></div><b>${escapeHtml(formatDistance(trip.distanceM))}</b><small>${escapeHtml(formatDuration(trip.durationMs))} • top ${Math.round(Number(trip.topKmh)||0)} km/h</small></article>`).join(''):'<div class="nx-empty">No recorded drives yet.</div>';
  };
