...
export function ensureStudioStyles() {
  ...
  style.textContent = `
    ...
    .nx-studio__chrome{height:100%;display:grid;grid-template-rows:auto auto minmax(0,1fr);gap:8px}
    ...
    .nx-studio__panel{display:none;height:100%;min-height:0;padding:10px;box-sizing:border-box}.nx-studio__panel.is-active{display:grid;align-content:stretch}.nx-studio__panel>.nx-studio-grid{height:100%;min-height:0}
    .nx-studio-grid{display:grid;gap:8px}.nx-studio-grid.two{grid-template-columns:minmax(0,1.15fr) minmax(0,.85fr)}.nx-studio-grid.rows{grid-template-rows:minmax(0,1fr) auto}.nx-studio-card{min-width:0;min-height:0;padding:9px;border:1px solid rgba(154,181,206,.12);border-radius:15px;background:linear-gradient(180deg,rgba(25,47,66,.88),rgba(9,24,37,.9));box-shadow:inset 0 1px 0 rgba(255,255,255,.08),inset 0 -2px 5px rgba(0,0,0,.26),0 6px 12px rgba(0,0,0,.12)}
    .nx-studio-card strong{color:#eff8ff;font-size:9px}.nx-studio-card p,.nx-studio-note{margin:4px 0 0;color:#8ea2b5;font-size:7.2px;line-height:1.32}.nx-studio-field{display:grid;gap:4px}.nx-studio-field>span{color:#97aabd;font-size:6.8px;font-weight:850;letter-spacing:.035em}.nx-studio input,.nx-studio textarea,.nx-studio select{width:100%;min-width:0;box-sizing:border-box;border:1px solid rgba(142,172,198,.14);border-radius:10px;background:linear-gradient(180deg,#07131f,#04101a);box-shadow:inset 0 3px 8px rgba(0,0,0,.48),inset 0 -1px 0 rgba(255,255,255,.035);color:#edf7ff;font-size:8px;outline:none}.nx-studio input,.nx-studio select{height:34px;padding:0 9px}.nx-studio textarea{height:72px;padding:8px;resize:none}.nx-studio input:focus,.nx-studio textarea:focus,.nx-studio select:focus{border-color:color-mix(in srgb,var(--studio) 45%,transparent);box-shadow:inset 0 3px 8px rgba(0,0,0,.48),0 0 0 2px color-mix(in srgb,var(--studio) 10%,transparent)}
...
export async function aiModel(systemInstruction='Be accurate. Do not invent missing information.') {
  if (!aiPromise) {
    aiPromise = import('https://www.gstatic.com/firebasejs/12.1.0/firebase-ai.js').then(ai => ({ ai, get:modelName => ai.getGenerativeModel(ai.getAI(firebaseApp,{backend:new ai.GoogleAIBackend()}),{model:modelName,systemInstruction:{parts:[{text:systemInstruction}]},generationConfig:{temperature:.3,maxOutputTokens:1800}}) })).catch(error => { aiPromise=null; throw error; });
  }
  const holder = await aiPromise;
  return holder.get('gemini-3.6-flash');
}
