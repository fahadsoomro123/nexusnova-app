...
function weatherMeta(code) {
  ...
  if ([61, 63, 65, 66, 67, 80, 81, 82].includes(n)) return { key: 'rain', label: n === 65 || n === 82 ? 'Heavy rain' : 'Rain' };
...
function weatherIcon(code) {
  ...
  return ({ clear:'☀', clouds:'◌', fog:'≋', rain:'⌁', snow:'❄', storm:'ϟ' })[key] || '◌';
}
...
function windyUrl(latitude, longitude, overlay = 'radar') {
  const params = new URLSearchParams({
    lat: String(Number(latitude).toFixed(4)),
    lon: String(Number(longitude).toFixed(4)),
    detailLat: String(Number(latitude).toFixed(4)),
    detailLon: String(Number(longitude).toFixed(4)),
    zoom: overlay === 'radar' || overlay === 'satellite' ? '7' : '5',
    level: 'surface',
    overlay,
    product: 'ecmwf',
    menu: '',
...
export function renderWeatherPremium() {
  const root = node(`
    <section class="nxwx-shell" data-wx-shell data-condition="clouds" data-phase="day">
      ...
      <section class="nxwx-hero">
        <div class="nxwx-atmosphere" aria-hidden="true">
          <i class="nxwx-sun"></i>
          <i class="nxwx-moon"></i>
          <i class="nxwx-stars"></i>
          <i class="nxwx-cloud nxwx-cloud--one"></i>
          <i class="nxwx-cloud nxwx-cloud--two"></i>
          <i class="nxwx-cloud nxwx-cloud--three"></i>
          <i class="nxwx-rain"></i>
          <i class="nxwx-snow"></i>
          <i class="nxwx-lightning"></i>
          <i class="nxwx-fog"></i>
        </div>
        <div class="nxwx-hero__content">
          ...
          <div class="nxwx-hero__badge">
            <span data-wx-icon>◌</span>
            <small data-wx-localtime>—</small>
          </div>
        </div>
      </section>
      ...
      <section class="nxwx-forecast" data-wx-days></section>
      ...
      <section class="nxwx-radar" data-wx-radar-shell>
        ...
        <div class="nxwx-layers" data-wx-layers>
          ${[
            ['radar','RADAR'], ['rain','RAIN'], ['wind','WIND'], ['clouds','CLOUDS'],
            ['temp','TEMP'], ['pressure','PRESSURE'], ['satellite','SATELLITE']
          ].map(([id, label], index) => `<button type="button" data-wx-layer="${id}" class="${index === 0 ? 'is-active' : ''}">${label}</button>`).join('')}
        </div>
        <div class="nxwx-mapframe">
          <iframe data-wx-radar title="Live interactive weather map" loading="lazy" allow="geolocation" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>
        <footer>Interactive live map by Windy • select a layer above • map controls remain available inside the panel.</footer>
      </section>
  ...
  const refs = Object.fromEntries([
    'place','phase','temp','condition','hi-low','icon','localtime','feels','humidity',
    'wind','winddir','gust','pressure','clouds','days'
  ].map(key => [key, root.querySelector(`[data-wx-${key}]`)]));
  ...
  const loadWeather = async ({ lat, lon, place }) => {
    ...
    try {
      ...
      refs.icon.textContent = weatherIcon(current.weather_code);
      ...
      refs['hi-low'].textContent = `H ${Math.round(Number(max) || 0)}°  •  L ${Math.round(Number(min) || 0)}°`;

      try {
        refs.localtime.textContent = new Intl.DateTimeFormat([], {
      ...
      refs.days.innerHTML = times.map((date, index) => {
        const label = index === 0 ? 'TODAY' : new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { weekday:'short' }).toUpperCase();
        const code = data.daily.weather_code?.[index];
        const rain = Math.round(Number(data.daily.precipitation_probability_max?.[index]) || 0);
        return `<article>
          <span>${escapeHtml(label)}</span>
          <b>${weatherIcon(code)}</b>
          <strong>${Math.round(Number(data.daily.temperature_2m_max?.[index]) || 0)}°</strong>
          <small>${Math.round(Number(data.daily.temperature_2m_min?.[index]) || 0)}° • ${rain}% rain</small>
        </article>`;
      }).join('');

      paintRadar();
      status.textContent = `Updated ${new Date().toLocaleTimeString([], { hour:'2-digit', minute:'2-digit' })} • ${escapeHtml(meta.label)} • live location weather`;
    } catch (error) {
  ...
  const findPlaces = async () => {
    ...
    try {
      ...
      status.textContent = `${searchRows.length} matching locations found. Select one.`;
      results.focus();
  ...
  currentPosition({ enableHighAccuracy:false }).then(position => {
    loadWeather({ lat:position.coords.latitude, lon:position.coords.longitude, place:'Current location' });
  }).catch(() => {
    loadWeather(state);
  });
