...
const RESET_KEY = 'nexusnova_travel_reset_20260912_v1';
...
function resetLegacyTravelDataOnce() {
  try {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      if (/^nexusnova_(?:travel_|trip_plan_)/.test(key)) staleKeys.push(key);
    }
    staleKeys.forEach(key => localStorage.removeItem(key));
    localStorage.setItem(RESET_KEY, '1');
  } catch {
    // Storage may be unavailable in a restricted WebView; Travel still renders.
  }
}
