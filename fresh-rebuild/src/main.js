...
function openMineBrandPortal() {
  try {
    ...
    window.NexusBrowserAndroid.postMessage(JSON.stringify({ action: 'open', url: 'https://nexusnovatools.com/' }));
    return true;
  } catch (error) {
...
const labels = { mine: ['MINE', 'mine'], hub: ['NOVA HUB', 'hub'] };
...
function setSplashMode(enabled) { document.body.classList.toggle('nx-splash-mode', Boolean(enabled)); }
function splashMarkup(phase = 'boot') {
  const entering = phase === 'entry';
  const kicker = entering ? 'IDENTITY VERIFIED / SECURE ACCESS' : 'NEXUSNOVA / PRIVATE DIGITAL INFRASTRUCTURE';
  const title = entering ? 'Secure workspace <span>online.</span>' : 'Infrastructure for your <span>digital world.</span>';
  const status = entering ? 'Synchronizing protected services' : 'Establishing secure session';
  return `<section class="nx-cinematic-splash" data-splash-phase="${entering ? 'entry' : 'boot'}" aria-label="NexusNova secure startup"><div class="nx-cinematic-splash__noise" aria-hidden="true"></div><div class="nx-cinematic-splash__beam" aria-hidden="true"></div><div class="nx-cinematic-splash__grid" aria-hidden="true"></div><div class="nx-cinematic-splash__orbital" aria-hidden="true"></div><div class="nx-cinematic-splash__shell"><div class="nx-cinematic-splash__telemetry" aria-hidden="true"><span>NX-01</span><span>ENCRYPTED LINK</span></div><div class="nx-cinematic-splash__core"><div class="nx-cinematic-splash__mark" aria-hidden="true"><span>N</span></div><p class="nx-cinematic-splash__kicker">${kicker}</p><h1>${title}</h1><div class="nx-cinematic-splash__rule" aria-hidden="true"><i></i></div><p class="nx-cinematic-splash__status"><span aria-hidden="true"></span>${status}</p></div><div class="nx-cinematic-splash__footer" aria-hidden="true"><span>SECURE CORE</span><span>NEXUSNOVA NETWORK</span></div></div></section>`;
}
function renderCinematicSplash(phase = 'boot') {
  showDock(false); setSplashMode(true); stage.dataset.route = 'splash'; stage.innerHTML = splashMarkup(phase); stage.focus({ preventScroll: true }); window.scrollTo({ top: 0, behavior: 'instant' });
}
function wait(ms) { return new Promise(resolve => setTimeout(resolve, Math.max(0, Number(ms) || 0))); }
function parentRouteForApp(id) { return mineAppIds.has(String(id || '')) ? 'mine' : 'hub'; }
...
const backToHub = () => { requestHubReturnRestore(); return router.render('hub'); };
...
function appModuleFailureScreen(id, error) {
  ...
  root.querySelector('[data-app-module-back]').addEventListener('click', () => { if (parent === 'mine') router.render('mine'); else backToHub(); });
  return root;
}

async function handleSignedIn() { renderCinematicSplash('entry'); await wait(POST_LOGIN_SPLASH_MS); await router.render('mine'); }

router = createRouter({
  stage,
  routes: {
    auth: async () => authScreen({ onSignedIn: handleSignedIn }),
    mine: async () => { cleanupActiveAppScreen(); cleanupMineScreen(); currentAppParent = 'mine'; return mineScreen({ openApp: openAppWithAd }); },
    hub: async () => { cleanupActiveAppScreen(); cleanupMineScreen(); currentAppParent = 'hub'; return hubScreen({ openApp: openAppWithAd }); },
    app: async payload => {
      cleanupActiveAppScreen(); cleanupMineScreen();
      const id = String(payload?.id || '');
      currentAppParent = parentRouteForApp(id);
      if (id === 'nova-vault') {
        try { const module = await loadNovaVaultModule(); return module.novaVaultScreen({ backToMine }); }
        catch (error) { return appModuleFailureScreen(id, error); }
      }
      try { const module = await loadAppScreenModule(); return module.appScreen({ id, backToHub, backToMine }); }
      catch (error) { return appModuleFailureScreen(id, error); }
    }
  },
  onBeforeRender: ({ route, payload }) => { setSplashMode(route === 'splash'); if (route !== 'splash') showDock(route !== 'auth'); syncDock(route, payload); },
  onAfterRender: ({ route }) => { if (route !== 'splash') stage.focus({ preventScroll: true }); }
});

dockItems.forEach(button => button.addEventListener('click', () => router.render(button.dataset.route || 'mine')));

(async function boot() {
  renderCinematicSplash('boot');
  try {
    const elapsed = performance.now() - bootSplashStartedAt;
    if (elapsed < BOOT_SPLASH_MIN_MS) await wait(BOOT_SPLASH_MIN_MS - elapsed);
    const user = await authService.restore();
    if (user) await router.render('mine'); else await router.render('auth');
  } catch (error) {
