...
function saneSpeed(value) {
  const n = Number(value);
  return Number.isFinite(n) && n >= 0 && n <= MAX_UI_SPEED_KMH ? Math.round(n) : 0;
}
...
function attach(root) {
  ...
  function paintDrive() {
    ...
    const speed = saneSpeed(splitMetric(text(root,'[data-dr-value]','0'),'km/h').value);
    setValue(q('[data-approved-speed]'), String(speed), 'km/h');
    ...
    setValue(q('[data-approved-top]'), String(saneSpeed(top.value)), top.unit || 'km/h');
  ...
  function paintVehicle() {
    ...
    setValue(q('[data-tracker-speed]'), String(saneSpeed(live?.speedKmh)), 'km/h');
