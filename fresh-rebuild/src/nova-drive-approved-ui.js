...
function attach(root) {
  ...
  const driveMirror = new MutationObserver(paintDrive);
  driveMirror.observe(root, { subtree:true, childList:true, characterData:true, attributes:true, attributeFilter:['class'] });
  paintDrive();
  checkEntitlement();
  ...
  const cleanupTimer = setInterval(() => {
    ...
    disposed = true;
    clearInterval(cleanupTimer); clearInterval(polling); clearTimeout(notify.timer); resetHold(); driveMirror.disconnect();
