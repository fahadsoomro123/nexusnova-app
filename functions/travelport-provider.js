...
function cfg(env=process.env){
  ...
  const configured=['TRAVELPORT_USERNAME','TRAVELPORT_PASSWORD','TRAVELPORT_CLIENT_ID','TRAVELPORT_CLIENT_SECRET','TRAVELPORT_ACCESS_GROUP'].every(k=>String(env[k]||'').trim());
...
async function token(env=process.env){
  ...
  const response=await fetch(c.tokenUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Accept':'application/json'},body,signal:AbortSignal.timeout(15000)}).catch(e=>{throw new TravelportError('provider_timeout','Travelport authentication timed out.',504)});
  const data=await response.json().catch(()=>null);
  if(!response.ok||!data?.access_token)throw new TravelportError(response.status===401?'provider_auth':'provider_error','Travelport authentication failed.',response.status===401?502:502);
  tokenCache={token:String(data.access_token),expiresAt:Date.now()+Math.max(60,Number(data.expires_in||86400)-300)*1000};
...
function normalize(data){
  const root=data?.CatalogProductOfferingsResponse||{}; const cpo=root.CatalogProductOfferings||{}; const refs=refIndex(data); const offerings=Array.isArray(cpo.CatalogProductOffering)?cpo.CatalogProductOffering:[];
  const results=[];
