/* NexusNova NEWS FIX V6
   Self-contained bilingual premium newsroom.
   It rebuilds only the News tab content instead of depending on the old malformed
   nested News markup, while preserving ALL APPS back navigation.
*/
(() => {
  "use strict";
  if (window.__nxPremiumNewsV6) return;
  ...
  const TEXT = {
    en:{
      ...
      status:"English News Desk", empty:"No stories are available in this section right now.",
    ...
    ur:{
      ...
      status:"اردو نیوز ڈیسک", empty:"اس حصے میں اس وقت کوئی خبر دستیاب نہیں۔",
  ...
  function splitTitle(title,fallbackSource){
    const parts = String(title || "").trim().split(/\s+-\s+/);
    if(parts.length > 1){
      const source = parts.pop().trim();
      return {title:parts.join(" - ").trim(),source:source || fallbackSource};
    }
    return {title:String(title || "").trim(),source:fallbackSource};
  }
  ...
  function timeAgo(value){
    ...
    if(language === "ur"){
      ...
      const h=Math.floor(mins/60); if(h<24) return `${h} گھنٹے پہلے`;
  ...
  function isBreaking(item,index){
    ...
    return (d && Date.now()-d.getTime() <= 6*3600e3) || index < 4;
  }

  function injectStyle(){
    if($("nxPremiumNewsStyleV6")) return;
    ["nxPremiumNewsStyle","nxPremiumNewsStyleV5"].forEach(id => $(id)?.remove());
    const style=document.createElement("style");
    style.id="nxPremiumNewsStyleV6";
    style.textContent=`
  ...
  function ensureRoot(){
    ...
    let root=$("nxNewsRoot");
    if(!root){
      // Preserve navigation controls but remove every legacy News card. The old
      // HTML has nested duplicate cards/headers and was the cause of the blank V5 UI.
      const keep=[];
      ...
      root.querySelectorAll("[data-news-lang]").forEach(btn => btn.addEventListener("click",() => {
        ...
        language=next; activeCategory="ALL"; localStorage.setItem("nx_news_language",language);
        updateShell(); window.loadNews();
      }));
  ...
  function updateShell(){
    const tab=$("tab-news"); if(!tab) return;
    tab.classList.toggle("nx-news-urdu",language === "ur");
    ...
    if(filters){
      ...
      Object.entries(t.categories).forEach(([cat,label]) => {
        const btn=document.createElement("button"); btn.type="button"; btn.className="nx-news-chip"+(cat===activeCategory?" active":""); btn.textContent=label;
        btn.addEventListener("click",() => { activeCategory=cat; updateShell(); render(currentItems); });
  ...
  function media(item,feature=false){
    ...
    const cls=feature?"nx-feature-media":"nx-news-thumb", fb=feature?"nx-feature-fallback":"nx-news-image-fallback";
    const initials=hasUrdu(item.source)?"خبر":String(item.source||"NN").split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("").toUpperCase();
    ...
    return `<div class="${fb}">${esc(initials || "NN")}</div>`;
  }
  function renderTicker(items){ const el=$("nxBreakingTrack"); if(!el) return; el.innerHTML=items.slice(0,14).map(x=>`<span><b>${esc(x.source)}</b>${esc(x.title)}</span>`).join("") || `<span>${language==="ur"?"تازہ خبریں لوڈ ہو رہی ہیں…":"Loading latest headlines…"}</span>`; }
  function render(items){
    ensureRoot(); const list=$("newsList"); if(!list) return;
    ...
    for(const raw of items){
      ...
      if(!key || seen.has(key)) continue; seen.add(key); unique.push(raw); if(unique.length>=64) break;
    }
    currentItems=unique; renderTicker(unique);
    const t=TEXT[language];
    const clean=activeCategory==="ALL"?unique:unique.filter(x=>x.category===activeCategory);
    if(!clean.length){ list.innerHTML=`<div class="nx-news-empty" ${language==="ur"?'dir="rtl"':""}>${esc(t.empty)}</div>`; return; }
    const lead=clean[0], rest=clean.slice(1,25);
    ...
    list.querySelectorAll("[data-i]").forEach(card=>card.addEventListener("click",()=>{const item=clean[Number(card.dataset.i)||0],url=safeUrl(item?.url);if(url)window.open(url,"_blank","noopener,noreferrer");}));
  }

  function feeds(lang){
    const g=(q,category,source)=>({rss:`https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=${lang==="ur"?"ur":"en-PK"}&gl=PK&ceid=PK:${lang==="ur"?"ur":"en"}`,category,source});
    return lang==="ur" ? [g("پاکستان","PAKISTAN","پاکستان نیوز"),g("سندھ کراچی حیدرآباد سکھر","SINDH","سندھ نیوز"),g("پاکستان کاروبار معیشت روپے ڈالر","BUSINESS","کاروباری خبریں"),g("پاکستان کرکٹ کھیل","SPORTS","کھیل"),g("پاکستان ٹیکنالوجی موبائل انٹرنیٹ","TECH","ٹیکنالوجی"),g("پاکستان شوبز فلم ڈرامہ","ENTERTAINMENT","شوبز"),g("عالمی خبریں دنیا","WORLD","عالمی خبریں")] : [g("Pakistan","PAKISTAN","Pakistan News"),g("Sindh Karachi Hyderabad Sukkur","SINDH","Sindh News"),g("Pakistan business economy rupee dollar","BUSINESS","Business"),g("Pakistan cricket sports","SPORTS","Sports"),g("Pakistan technology mobile internet","TECH","Technology"),g("Pakistan entertainment film drama","ENTERTAINMENT","Entertainment"),g("world news","WORLD","World News")];
  }
  async function fetchJson(url){ const c=new AbortController(),tm=setTimeout(()=>c.abort(),12000); try{const r=await fetch(url,{cache:"no-store",signal:c.signal});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json();}finally{clearTimeout(tm);} }
  async function rss2json(feed){
    const data=await fetchJson("https://api.rss2json.com/v1/api.json?rss_url="+encodeURIComponent(feed.rss));
    if(data?.status && data.status!=="ok") throw new Error(data.message||"RSS error");
    return (Array.isArray(data.items)?data.items:[]).map(a=>{const p=splitTitle(a.title,feed.source),html=a.content||a.description||"";return {title:p.title,url:a.link||"",date:a.pubDate||"",source:p.source||feed.source,image:safeUrl(a.thumbnail||a.enclosure?.link||imageFromHtml(html)),description:plainText(html).slice(0,260),category:feed.category};});
  }
  async function allOrigins(feed){
    const c=new AbortController(),tm=setTimeout(()=>c.abort(),12000);try{const r=await fetch("https://api.allorigins.win/raw?url="+encodeURIComponent(feed.rss),{cache:"no-store",signal:c.signal});if(!r.ok)throw new Error(`HTTP ${r.status}`);const xml=await r.text(),doc=new DOMParser().parseFromString(xml,"text/xml");return [...doc.querySelectorAll("item")].map(item=>{const p=splitTitle(item.querySelector("title")?.textContent||"",feed.source),desc=item.querySelector("description")?.textContent||"",img=item.querySelector("enclosure")?.getAttribute("url")||imageFromHtml(desc);return {title:p.title,url:item.querySelector("link")?.textContent||"",date:item.querySelector("pubDate")?.textContent||"",source:p.source||feed.source,image:safeUrl(img),description:plainText(desc).slice(0,260),category:feed.category};});}finally{clearTimeout(tm);}
  }
  async function gather(){
    const fs=feeds(language); const first=await Promise.allSettled(fs.map(rss2json)); let merged=first.flatMap(x=>x.status==="fulfilled"?x.value:[]);
    if(merged.length<5){const fb=await Promise.allSettled(fs.map(allOrigins));merged=merged.concat(fb.flatMap(x=>x.status==="fulfilled"?x.value:[]));}
    return merged;
  }

  window.loadNews=async function(){
    if(!ensureRoot()) return; const list=$("newsList"),id=++requestNo,t=TEXT[language]; if(!list)return;
    list.innerHTML=`<div class="nx-news-loading" ${language==="ur"?'dir="rtl"':""}>${esc(t.loading)}</div>`;
    try{const items=await gather();if(id!==requestNo)return;if(!items.length)throw new Error("No stories");render(items);}catch(err){console.warn("NexusNova News V6",err);if(id!==requestNo)return;list.innerHTML=`<div class="nx-news-empty" ${language==="ur"?'dir="rtl"':""}>${esc(t.offline)}</div>`;}
  };
