/* NexusNova NEWS FIX V4
   Premium Pakistan-first live newsroom with images, breaking ticker,
   Pakistan/World categories and real Urdu/English source switching.
*/
(() => {
  "use strict";
  if (window.__nxPremiumNewsV4) return;
  window.__nxPremiumNewsV4 = true;
  // Keep compatibility with integrity guards created for V3.
  ...
  const TEXT = {
    en: {
      ...
      sub:"Pakistan-first headlines • Sindh • business • sports • world • English edition",
    ...
    ur: {
      ...
      sub:"پاکستان • سندھ • کاروبار • کھیل • دنیا • مکمل اردو ایڈیشن",
  ...
  function categoryOf(item) {
    if (item.scope === "WORLD") return "WORLD";
    const t = `${item.title || ""} ${item.description || ""}`.toLowerCase();
    if (/karachi|sindh|hyderabad|sukkur|larkana|shikarpur|thar|mirpurkhas|کراچی|سندھ|حیدرآباد|سکھر|لاڑکانہ|شکارپور|تھر|میرپورخاص/.test(t)) return "SINDH";
    if (/cricket|psl|football|hockey|match|t20|odi|babar|shaheen|کرکٹ|پی ایس ایل|فٹبال|ہاکی|میچ|بابر|شاہین/.test(t)) return "SPORTS";
    if (/business|economy|rupee|dollar|gold|petrol|diesel|stock|psx|imf|sbp|tax|budget|trade|کاروبار|معیشت|روپیہ|ڈالر|سونا|پیٹرول|اسٹاک|آئی ایم ایف|بجٹ|تجارت/.test(t)) return "BUSINESS";
    if (/technology|tech|artificial intelligence|software|mobile|internet|cyber|space|satellite|ٹیکنالوجی|مصنوعی ذہانت|سافٹ ویئر|موبائل|انٹرنیٹ|سائبر|خلائی|سیٹلائٹ/.test(t)) return "TECH";
    if (/film|drama|actor|actress|music|showbiz|entertainment|celebrity|فلم|ڈرامہ|اداکار|اداکارہ|موسیقی|شوبز/.test(t)) return "ENTERTAINMENT";
    return "PAKISTAN";
  }
  ...
  function articleMedia(item, cls) {
    ...
    const fallback = cls === "nx-feature-media" ? "nx-feature-fallback" : "nx-news-image-fallback";
    if (img) {
      return `<img class="${cls}" src="${esc(img)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="${fallback}" style="display:none">${hashFallback(item.source)}</div>`;
    }
    return `<div class="${fallback}">${hashFallback(item.source)}</div>`;
  }
  ...
  function injectPremiumStyle() {
    ...
    style.textContent = `
      ...
      .nx-news-language-bar{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:0 0 10px;padding:9px 10px;border:1px solid rgba(56,189,248,.16);background:rgba(8,15,28,.78);border-radius:12px}
      .nx-news-language-label{font-size:10px;color:#94a3b8;font-weight:800}
      .nx-news-language-switch{display:flex;gap:6px}
      .nx-news-lang-btn{border:1px solid rgba(148,163,184,.2);background:#111827;color:#cbd5e1;border-radius:9px;padding:7px 12px;font-size:11px;font-weight:900}
      .nx-news-lang-btn.active{background:linear-gradient(135deg,#0ea5e9,#2563eb);border-color:transparent;color:#fff;box-shadow:0 8px 20px rgba(37,99,235,.22)}
      ...
      #tab-news.nx-news-urdu .nx-feature-title,#tab-news.nx-news-urdu .nx-news-card-title,#tab-news.nx-news-urdu .nx-news-snippet{direction:rtl;text-align:right;font-family:"Noto Nastaliq Urdu","Noto Sans Arabic","Segoe UI",Arial,sans-serif}
      #tab-news.nx-news-urdu .nx-feature-meta,#tab-news.nx-news-urdu .nx-card-meta{direction:rtl;justify-content:flex-start}
      #tab-news.nx-news-urdu .nx-breaking-track{direction:rtl}
      #tab-news.nx-news-urdu .nx-breaking-track span{font-family:"Noto Nastaliq Urdu","Noto Sans Arabic","Segoe UI",Arial,sans-serif}
      #tab-news.nx-news-urdu .nx-news-read{direction:rtl}
  ...
  function updateShell() {
    ...
    Object.keys(t.categories).forEach(cat => {
      ...
      b.textContent = t.categories[cat];
  ...
  function render(items) {
    ...
    for (const raw of items) {
      ...
      // Unicode-safe key: old V3's a-z-only cleanup discarded Urdu titles.
      ...
      unique.push({...raw,category:raw.category || categoryOf(raw)});
  ...
  async function rss2json(rss, sourceHint = "", scope = "PAKISTAN") {
    ...
    return (Array.isArray(data.items) ? data.items : []).map(a => {
      const parsed = splitGoogleTitle(a.title, sourceHint || a.author || "News");
      const description = plainText(a.description || a.content || "").slice(0, 260);
      const image = safeHttpUrl(a.thumbnail || a.enclosure?.link || imageFromHtml(a.content || a.description));
      return {
        title:parsed.title,
        url:a.link || a.url || "",
        date:a.pubDate,
        source:parsed.source || sourceHint || "News",
        image,
        description,
        scope
      };
  ...
  async function allOriginsRss(rss, sourceHint = "", scope = "PAKISTAN") {
    ...
    try {
      ...
      return [...doc.querySelectorAll("item")].map(item => {
        const parsed = splitGoogleTitle(item.querySelector("title")?.textContent || "", sourceHint || "News");
  ...
  function feedsForLanguage(lang) {
    if (lang === "ur") {
      return [
        {rss:"https://news.google.com/rss/search?q=" + encodeURIComponent("پاکستان") + "&hl=ur&gl=PK&ceid=PK:ur",source:"پاکستان نیوز",scope:"PAKISTAN"},
        {rss:"https://news.google.com/rss/search?q=" + encodeURIComponent("سندھ کراچی") + "&hl=ur&gl=PK&ceid=PK:ur",source:"سندھ نیوز",scope:"PAKISTAN"},
        {rss:"https://news.google.com/rss/search?q=" + encodeURIComponent("دنیا عالمی خبریں") + "&hl=ur&gl=PK&ceid=PK:ur",source:"عالمی خبریں",scope:"WORLD"}
      ];
    }

    return [
      {rss:"https://news.google.com/rss/search?q=" + encodeURIComponent("Pakistan") + "&hl=en-PK&gl=PK&ceid=PK:en",source:"Google News Pakistan",scope:"PAKISTAN"},
      {rss:"https://tribune.com.pk/feed/pakistan",source:"Express Tribune",scope:"PAKISTAN"},
      {rss:"https://news.google.com/rss/search?q=" + encodeURIComponent("world news") + "&hl=en-PK&gl=PK&ceid=PK:en",source:"World News",scope:"WORLD"}
    ];
  }

  async function gatherSources() {
    const feeds = feedsForLanguage(language);
    const first = await Promise.allSettled(feeds.map(feed => rss2json(feed.rss, feed.source, feed.scope)));
    let merged = first.flatMap(result => result.status === "fulfilled" ? result.value : []);

    if (!merged.length) {
      const fallback = await Promise.allSettled(feeds.map(feed => allOriginsRss(feed.rss, feed.source, feed.scope)));
      merged = fallback.flatMap(result => result.status === "fulfilled" ? result.value : []);
    }
    return merged;
  }
  ...
  window.loadNews = async function() {
    ...
    try {
      ...
      console.warn("NexusNova bilingual premium news:", error);
      if (id !== requestNo) return;
