...
try{
  ...
  const refineAfter=await execute('const c=document.querySelector(".nxqt-canvas-wrap canvas:not([hidden])");return c.getContext("2d").getImageData(30,50,1,1).data[3];');record('Quick Tools','Remove BG Erase brush changes alpha',refineAfter<refineBefore-80,`${refineBefore} → ${refineAfter}`);
  await click('[data-nxqt-refine-undo]');await delay(120);const refineUndo=await execute('const c=document.querySelector(".nxqt-canvas-wrap canvas:not([hidden])");return c.getContext("2d").getImageData(30,50,1,1).data[3];');record('Quick Tools','Remove BG refine Undo restores subject',Math.abs(refineUndo-refineBefore)<8,`${refineAfter} → ${refineUndo}`);
  await click('[data-nxqt-refine-reset]');await delay(80);await expect('Quick Tools','Remove BG Reset Auto remains available','return !document.querySelector("[data-nxqt-refine-undo]").disabled&&document.querySelector("[data-nxqt-refine-feather]").value==="0";');
