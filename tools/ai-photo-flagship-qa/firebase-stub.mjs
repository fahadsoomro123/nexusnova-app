...
export const getGenerativeModel=(_ai,options={})=>({
  async generateContent(parts=[]){
    ...
    const system=Array.isArray(options.systemInstruction?.parts)
      ?options.systemInstruction.parts.map(part=>String(part?.text||'')).join(' ')
      :String(options.systemInstruction||'');
    if(/Return only requested JSON adjustment values/i.test(system)){
      return{response:{text:()=>JSON.stringify({exposure:.35,brightness:8,contrast:14,highlights:-18,shadows:16,whites:6,blacks:-5,temperature:7,tint:2,vibrance:13,saturation:4,clarity:9,dehaze:5,sharpness:17,noiseReduction:18,vignette:5,fade:2})}};
    }
