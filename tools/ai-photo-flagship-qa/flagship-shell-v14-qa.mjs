...
try{
  ...
  await expect('Bottom editor option labels are visible','const nodes=[...document.querySelectorAll(".nx-photo-tool span")];const rgb=s=>(s.match(/\\d+(?:\\.\\d+)?/g)||[]).slice(0,3).map(Number);return nodes.length>=6&&nodes.every(n=>{const s=getComputedStyle(n),r=n.getBoundingClientRect(),c=rgb(s.color);return r.width>0&&r.height>0&&s.visibility!=="hidden"&&s.display!=="none"&&Number(s.opacity||1)>.6&&c.length===3&&c.reduce((a,v)=>a+v,0)>400});');
